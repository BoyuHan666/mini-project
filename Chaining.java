...
public class Chaining {
    ...
    public int chain (int key) {
    	...
    	int h = ((A*key)%(this.power2(w)))>>Math.abs(w-r);
        return h;
    }
    ...
    public int insertKey(int key){
        //TODO: implement this and change the return statement
    	int bk = chain(key);
    	// if the table is empty,collision equals to 0
    	if(this.Table.get(bk).isEmpty()){
    		this.Table.get(bk).add(key);
    		return 0;
    	}
    	// if the table is not empty,collision equals to the table size 
    	else{
    		int num = this.Table.get(bk).size();
    		this.Table.get(bk).add(key);
    		return num;
    	}
